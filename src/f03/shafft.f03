...
integer(c_int), parameter, public :: SHAFFT_TENSOR_LAYOUT_INITIAL = 1 ! Initial layout (before transforms)
integer(c_int), parameter, public :: SHAFFT_TENSOR_LAYOUT_TRANSFORMED = 2 ! Transformed layout (after transforms)
...
integer(c_int), parameter, public :: SHAFFT_ERR_INTERNAL = 12
...
! Legacy helper retained for compatibility; unused in wrappers.
...
!> @brief Compute an N-D slab decomposition with a specified number of distributed axes.
   ...
   subroutine shafftConfigurationNDA(ndim, size, nda, subsize, offset, COMM_DIMS, precision, mem_limit, comm, ierr)
   ...
   integer(c_int), dimension(ndim), target, intent(inout) :: COMM_DIMS
   ...
   rc = c_shafftConfigurationNDA(ndim, c_loc(size), nda, c_loc(subsize), c_loc(offset), &
...
subroutine shafftConfigurationCart(ndim, size, subsize, offset, &
                                   COMM_DIMS, COMM_SIZE, precision, mem_limit, comm, ierr)
   integer(c_int), intent(in) :: ndim
   ...
   integer(c_int), target, dimension(ndim), intent(inout) :: COMM_DIMS
   ...
   rc = c_shafftConfigurationCart(ndim, c_loc(size), c_loc(subsize), c_loc(offset), &
...
subroutine shafftPlanNDA(plan, ndim, nda, dimensions, precision, comm, ierr)
   integer(c_int), intent(in) :: ndim
   ...
   integer(c_int), intent(out) :: ierr
   integer(c_int) :: rc
   rc = c_shafftPlanCreate(plan)
   if (rc /= 0) then
      ierr = rc
      return
   end if
   rc = c_shafftPlanNDA(plan, ndim, nda, c_loc(dimensions), precision, comm)
   ierr = rc
...
subroutine shafftPlanCart(plan, COMM_DIMS, dimensions, precision, comm, ierr)
   type(c_ptr), intent(out) :: plan
   ...
   integer(c_int), intent(out) :: ierr
   integer(c_int) :: rc
   ndim = int(size(dimensions), c_int)
   rc = c_shafftPlanCreate(plan)
   if (rc /= 0) then
      ierr = rc
      return
   end if
   rc = c_shafftPlanCart(plan, ndim, c_loc(COMM_DIMS), c_loc(dimensions), precision, comm)
   ierr = rc
...
subroutine shafftGetAllocSize(plan, alloc_size, ierr)
   type(c_ptr), intent(in) :: plan
   integer(c_size_t), intent(out) :: alloc_size
   integer(c_int), intent(out) :: ierr
   ierr = c_shafftGetAllocSize(plan, alloc_size)
...
subroutine shafftGetBuffers_sp(plan, alloc_size, data, work, ierr)
   type(c_ptr), intent(in) :: plan
   integer(c_size_t), intent(in), optional :: alloc_size
...
subroutine shafftGetBuffers_dp(plan, alloc_size, data, work, ierr)
   type(c_ptr), intent(in) :: plan
   integer(c_size_t), intent(in), optional :: alloc_size
...
subroutine shafftErrorSourceName(source, name)
   ...
   character(kind=c_char), pointer :: cstr(:)
   integer :: i, slen
   cname = c_shafft_error_source_name(source)
   slen = c_strlen(cname)
   name = ''
   if (slen > 0) then
      call c_f_pointer(cname, cstr, [slen])
