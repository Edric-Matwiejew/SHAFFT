...
int shafftSetStream(void* plan_ptr, hipStream_t stream) {
  if (!plan_ptr) return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
...
static int shafftCTypeCast(shafft_t cType, shafft::FFTType* out) {
  if (!out) return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
  switch (cType) {
    case SHAFFT_C2C: *out = shafft::FFTType::C2C; return (int)shafft::Status::SHAFFT_SUCCESS;
    case SHAFFT_Z2Z: *out = shafft::FFTType::Z2Z; return (int)shafft::Status::SHAFFT_SUCCESS;
    default:         return (int)shafft::Status::SHAFFT_ERR_INVALID_FFTTYPE;
  }
}
...
static int fftCDirectionCast(shafft_direction_t cdir, shafft::FFTDirection* out) {
  if (!out) return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
  switch (cdir) {
    case SHAFFT_FORWARD:  *out = shafft::FFTDirection::FORWARD;  return (int)shafft::Status::SHAFFT_SUCCESS;
    case SHAFFT_BACKWARD: *out = shafft::FFTDirection::BACKWARD; return (int)shafft::Status::SHAFFT_SUCCESS;
    default:              return (int)shafft::Status::SHAFFT_ERR_INVALID_DIM; // or define a DIR error
  }
}
...
int shafftPlanCreate(void** out_plan) {
  if (!out_plan) return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
  *out_plan = nullptr;
  shafft::PlanData* p = nullptr;
  int rc = _shafft::planCreate(&p);
  if (rc == (int)shafft::Status::SHAFFT_SUCCESS) *out_plan = static_cast<void*>(p);
  return rc;
}
...
int shafftConfigurationNDA(int ndim, int* size, int* nda, int* subsize,
  ...
  if (rc != (int)shafft::Status::SHAFFT_SUCCESS) return rc;

  return _shafft::configurationNDA(ndim, size, nda, subsize, offset, COMM_DIMS,
                                   prec, mem_limit, c_comm);
}
...
int shafftConfigurationCart(int ndim, int* size, int* subsize, int* offset,
  ...
  return _shafft::configurationCart(ndim, size, subsize, offset, COMM_DIMS,
                                    COMM_SIZE, prec, mem_limit, c_comm);
}
...
int shafftGetLayout(void* plan_ptr, int* subsize, int* offset,
                    shafft_tensor_layout_t layout) {
  if (!plan_ptr) return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
  shafft::PlanData* plan = static_cast<shafft::PlanData*>(plan_ptr);
  return _shafft::getLayout(plan, subsize, offset,
                            static_cast<shafft::TensorLayout>(layout));
}

int shafftGetAxes(void* plan_ptr, int* ca, int* da,
                  shafft_tensor_layout_t layout) {
  if (!plan_ptr) return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
  shafft::PlanData* plan = static_cast<shafft::PlanData*>(plan_ptr);
  return _shafft::getAxes(plan, ca, da,
                          static_cast<shafft::TensorLayout>(layout));
}

// CHANGED: return status, take an out parameter
int shafftGetAllocSize(void* plan_ptr, size_t* alloc_size) {
  if (!plan_ptr || !alloc_size) return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
  shafft::PlanData* plan = static_cast<shafft::PlanData*>(plan_ptr);
...
int shafftAllocBufferF(size_t count, void** buf) {
  if (!buf) return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
...
int shafftCopyToBufferF(void* dst, const void* src, size_t count) {
  return shafft::copyToBuffer(
      static_cast<shafft::complexf*>(dst),
      static_cast<const shafft::complexf*>(src),
      count);
}

int shafftCopyToBufferD(void* dst, const void* src, size_t count) {
  return shafft::copyToBuffer(
      static_cast<shafft::complexd*>(dst),
      static_cast<const shafft::complexd*>(src),
      count);
}
