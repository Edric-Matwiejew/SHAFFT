...
static bool within_mem_limit_guarded(int ndim, const int size_per_rank[],
                                     shafft::FFTType precision,
                                     size_t mem_limit) noexcept {
  // mem_limit <= 0 => ignore memory constraint
  if (mem_limit == 0) return true;
  if (static_cast<long long>(mem_limit) < 0) return true;

  size_t type_size = 0;
  switch (precision) {
    case shafft::FFTType::C2C: type_size = 2 * sizeof(float); break;
    case shafft::FFTType::Z2Z: type_size = 2 * sizeof(double); break;
  ...
  __uint128_t elems = 1;
  for (int i = 0; i < ndim; ++i) {
    if (size_per_rank[i] <= 0) return false;
    elems *= static_cast<uint64_t>(size_per_rank[i]);
    if (elems > ((__uint128_t)mem_limit) / (buffer_count * type_size)) return false;
  }
...
static int allreduce_bool_and(bool local_ok, bool* global_ok, MPI_Comm comm) noexcept {
  if (!global_ok) return SHAFFT_STATUS(SHAFFT_ERR_NULLPTR);
...
static int largest_divisor_leq(int n, int limit) {
  ...
  for (int d = 2; d*d <= n; ++d) {
    if (n % d == 0) {
      int d1 = d;
      int d2 = n / d;
      if (d1 <= up) best = std::max(best, d1);
      if (d2 <= up) best = std::max(best, d2);
      if (best == up) return best; // can't do better than limit
    }
  }
  // If none of the proper factors fit, check n itself
  if (n <= up) best = std::max(best, n);
  return best;
}
...
static int pack_prefix_with_spill(int ndim, int* grid, const int* size, const int* caps /*nullable*/) {
  // Collect multiplicative factors in order
  std::vector<int> factors;
  factors.reserve(ndim);
  for (int i = 0; i < ndim; ++i) if (grid[i] > 1) factors.push_back(grid[i]);

  // Reset grid to 1s; we'll rebuild the prefix
  for (int i = 0; i < ndim; ++i) grid[i] = 1;
  ...
  auto axis_cap = [&](int i) -> int {
    int c = size[i];
    if (caps) c = std::min(c, caps[i]);
    return std::max(1, c);
  };

  std::vector<int> cap_left(max_axes, 1);
  for (int i = 0; i < max_axes; ++i) cap_left[i] = axis_cap(i);
  ...
  // Count leading >1 (d')
  int d_prime = 0;
  while (d_prime < max_axes && grid[d_prime] > 1) ++d_prime;

  // Ensure remaining entries are 1 (including last axis)
  for (int i = d_prime; i < ndim; ++i) grid[i] = 1;
  return d_prime;
}
...
static bool passes_limits(int ndim, const std::vector<int>& subsize,
                          shafft::FFTType precision, size_t mem_limit,
                          bool require_mem)
{
  if (!within_int_limit_exact(ndim, subsize.data())) return false;
  if (require_mem && !within_mem_limit_guarded(ndim, subsize.data(), precision, mem_limit))
    return false;
  return true;
}

static int compute_local_block_cart(int ndim, const int* global_size,
                                    int d, const int* grid,
                                    MPI_Comm COMM,
                                    std::vector<int>& subsize,
                                    std::vector<int>& offset,
                                    MPI_Comm* cart_out) noexcept
{
  subsize.assign(ndim, 0);
  offset.assign(ndim, 0);
  if (!cart_out) return SHAFFT_STATUS(SHAFFT_ERR_NULLPTR);
  *cart_out = MPI_COMM_NULL;

  if (d <= 0) {
    for (int i = 0; i < ndim; ++i) { subsize[i] = global_size[i]; offset[i] = 0; }
    return SHAFFT_STATUS(SHAFFT_SUCCESS);
  }

  int cart_size = 1;
  for (int i = 0; i < d; ++i) cart_size *= grid[i];
  ...
  if (sub == MPI_COMM_NULL) return SHAFFT_STATUS(SHAFFT_SUCCESS);

  std::vector<int> periods(d, 0);
  SHAFFT_MPI_OR_FAIL(MPI_Cart_create(sub, d, const_cast<int*>(grid), periods.data(), /*reorder=*/0, cart_out));
...
int configurationNDA(int ndim, int size[], int* nda,
                     int* subsize_out, int* offset_out,
                     int* COMM_DIMS,
                     shafft::FFTType precision,
                     size_t mem_limit,
                     MPI_Comm COMM) noexcept
{
  try {
    ...
    auto try_d = [&](int d, bool strict)->bool {
      if (d < 1 || d > ndim - 1) return false;
      if (create_comm_dims(ndim, world_size, d, size, COMM_DIMS) != 0) return false;
      ...
      if (strict) {
        int gt1 = 0;
        for (int i = 0; i < d; ++i) if (COMM_DIMS[i] > 1) ++gt1;
        if (gt1 != d) return false;
      }

      const int max_axes = std::max(0, ndim - 1);
      std::vector<int> caps; calc_min_caps(ndim, max_axes, size, caps);
      ...
      if (!strict) {
        ...
        if (d_effective == 0 && world_size > 1) return false;
      }
      // In strict mode, use the grid from create_comm_dims directly (no packing)

      std::vector<int> subsize, offset;
      MPI_Comm cart = MPI_COMM_NULL;
      int rc = compute_local_block_cart(ndim, size, d_effective, COMM_DIMS, COMM, subsize, offset, &cart);
      if (rc != SHAFFT_STATUS(SHAFFT_SUCCESS)) return false;

      bool local_ok = (cart == MPI_COMM_NULL) || passes_limits(ndim, subsize, precision, mem_limit, require_mem);
      bool global_ok = false;
      rc = allreduce_bool_and(local_ok, &global_ok, COMM);
      if (cart != MPI_COMM_NULL) { SHAFFT_MPI_OR_FAIL(MPI_Comm_free(&cart)); }
      if (rc != SHAFFT_STATUS(SHAFFT_SUCCESS)) return false;
    ...
    if (!auto_mode) {
      if (!try_d(*nda, true)) return SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);
    } else {
      if (want_max) {
        for (int d = ndim - 1; d >= 1; --d) if (try_d(d, false)) break;
      } else {
        for (int d = 1; d <= ndim - 1; ++d) if (try_d(d, false)) break;
      }
      if (best_d < 0) return SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);
    }
...
int configurationCart(int ndim, int size[],
                      int* subsize_out, int* offset_out,
                      int* COMM_DIMS, int* COMM_SIZE,
                      shafft::FFTType precision,
                      size_t mem_limit,
                      MPI_Comm COMM) noexcept
{
  try {
    ...
    auto validate_and_build = [&](int d, const int* grid,
                                  std::vector<int>& subsize, std::vector<int>& offset) -> int {
      std::vector<int> caps; calc_min_caps(ndim, d, size, caps);
      for (int i = 0; i < d; ++i) {
        if (grid[i] < 1)       return SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);
        if (grid[i] > size[i]) return SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);
        if (grid[i] > caps[i]) return SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);
      }
      const int cart_sz = prod_prefix(grid, d);
      if (cart_sz > world_size) return SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);

      MPI_Comm cart = MPI_COMM_NULL;
      int rc = compute_local_block_cart(ndim, size, d, grid, COMM, subsize, offset, &cart);
      if (rc != SHAFFT_STATUS(SHAFFT_SUCCESS)) return rc;
      ...
      return global_ok ? SHAFFT_STATUS(SHAFFT_SUCCESS)
                       : SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);
    };
    ...
    if (!all_zero(COMM_DIMS, ndim)) {
      ...
      if (d == 0) {
        ...
        bool all_ones = true;
        for (int i = 0; i < ndim; ++i) {
          if (COMM_DIMS[i] != 1) { all_ones = false; break; }
        }
        ...
        if (d >= ndim) return SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);
        for (int i = d; i < ndim; ++i) {
          if (COMM_DIMS[i] > 1)              return SHAFFT_STATUS(SHAFFT_ERR_INVALID_DECOMP);
          if (COMM_DIMS[i] == 0) COMM_DIMS[i] = 1; // normalize trailing zeros
        }
      ...
      if (want_max) {
        for (int d = ndim - 1; d >= 1; --d) { rc = try_d(d); if (rc == SHAFFT_STATUS(SHAFFT_SUCCESS)) break; }
      } else {
        for (int d = 1; d <= ndim - 1; ++d) { rc = try_d(d); if (rc == SHAFFT_STATUS(SHAFFT_SUCCESS)) break; }
      }
      if (best_d < 0) return rc;
    }
