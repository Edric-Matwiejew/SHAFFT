...
static void dbg_hdr(const char* where) {
  if (!diag_enabled())
    return;
  int r = mpi_rank();
  std::cerr << "[SHAFFT:fftw]" << (r >= 0 ? ("[r" + std::to_string(r) + "]") : std::string("[r?]"))
            << "[" << where << "] ";
}
...
static void build_guru64_dims(int ndim, const int* dims, int a, int b, std::vector<fftw_iodim64>& t,
                              std::vector<fftw_iodim64>& blist) {
  std::vector<long long> P(ndim + 1, 1);
  for (int m = ndim - 1; m >= 0; --m)
    P[m] = P[m + 1] * (long long)dims[m];
...
int fftPlan(fftHandle& plan, int nta, int* ta, int ndim, int* dimensions,
  ...
  if (precision == shafft::FFTType::Z2Z) {
    ...
    if (!dummy_dp)
      throw std::runtime_error("FFTW: dummy allocation failed (double)");
  } else {
    dummy_sp = (fftwf_complex*)fftwf_malloc(total_elems * sizeof(fftwf_complex));
    if (!dummy_sp)
      throw std::runtime_error("FFTW: dummy allocation failed (float)");
  }

  if (precision == shafft::FFTType::Z2Z) {
    plan.fwd_dp = fftw_plan_guru64_dft((int)t.size(), t.data(), (int)blist.size(), blist.data(),
                                       dummy_dp, dummy_dp, FFTW_FORWARD, plan.flags);
    plan.bwd_dp = fftw_plan_guru64_dft((int)t.size(), t.data(), (int)blist.size(), blist.data(),
                                       dummy_dp, dummy_dp, FFTW_BACKWARD, plan.flags);
    fftw_free(dummy_dp);
    ...
    plan.fwd_sp = fftwf_plan_guru64_dft((int)tf.size(), tf.data(), (int)bf.size(), bf.data(),
                                        dummy_sp, dummy_sp, FFTW_FORWARD, plan.flags);
    plan.bwd_sp = fftwf_plan_guru64_dft((int)tf.size(), tf.data(), (int)bf.size(), bf.data(),
                                        dummy_sp, dummy_sp, FFTW_BACKWARD, plan.flags);
    fftwf_free(dummy_sp);
...
int fftExecute(fftHandle plan, void*& idata, void*& odata, shafft::FFTDirection direction) {
  ...
  if (plan.fft_type == shafft::FFTType::Z2Z) {
    if (direction == shafft::FFTDirection::FORWARD)
      fftw_execute_dft(plan.fwd_dp, reinterpret_cast<fftw_complex*>(idata),
                       reinterpret_cast<fftw_complex*>(odata));
    else
      fftw_execute_dft(plan.bwd_dp, reinterpret_cast<fftw_complex*>(idata),
                       reinterpret_cast<fftw_complex*>(odata));
  } else {
    if (direction == shafft::FFTDirection::FORWARD)
      fftwf_execute_dft(plan.fwd_sp, reinterpret_cast<fftwf_complex*>(idata),
                        reinterpret_cast<fftwf_complex*>(odata));
    else
      fftwf_execute_dft(plan.bwd_sp, reinterpret_cast<fftwf_complex*>(idata),
                        reinterpret_cast<fftwf_complex*>(odata));
  }
