...
int Plan::init(int nda, const std::vector<int>& dimensions,
...
int Plan::initCart(const std::vector<int>& commDims,
...
int configurationNDA(const std::vector<int> &size, int &nda,
                     ...
                     std::vector<int> &COMM_DIMS, shafft::FFTType precision,
                     size_t mem_limit, MPI_Comm COMM) {
  const int ndim = static_cast<int>(size.size());
  if ((int)subsize.size() != ndim || (int)offset.size() != ndim ||
      (int)COMM_DIMS.size() != ndim) {
    return (int)shafft::Status::SHAFFT_ERR_DIM_MISMATCH;
  }
  return _shafft::configurationNDA(
      ndim, const_cast<int *>(size.data()), &nda, subsize.data(), offset.data(),
      COMM_DIMS.data(), precision, mem_limit, COMM);
}

int configurationCart(const std::vector<int> &size, std::vector<int> &subsize,
                      std::vector<int> &offset, std::vector<int> &COMM_DIMS,
                      int &COMM_SIZE, shafft::FFTType precision,
                      size_t mem_limit, MPI_Comm COMM) {
  const int ndim = static_cast<int>(size.size());
  if ((int)subsize.size() != ndim || (int)offset.size() != ndim ||
      (int)COMM_DIMS.size() != ndim) {
  ...
  return _shafft::configurationCart(
      ndim, const_cast<int *>(size.data()), subsize.data(), offset.data(),
      COMM_DIMS.data(), &COMM_SIZE, precision, mem_limit, COMM);
}

int planNDA(PlanData *plan, int nda, const std::vector<int> &dimensions,
            shafft::FFTType precision, MPI_Comm COMM) {
  const int ndim = static_cast<int>(dimensions.size());
  return _shafft::planNDA(plan, ndim, nda, const_cast<int *>(dimensions.data()),
                          precision, COMM);
}

int planCart(PlanData *plan, const std::vector<int> &COMM_DIMS,
             const std::vector<int> &dimensions, shafft::FFTType precision,
             MPI_Comm COMM) {
  const int ndim = static_cast<int>(COMM_DIMS.size());
  if (ndim != (int)dimensions.size()) {
    return (int)shafft::Status::SHAFFT_ERR_DIM_MISMATCH;
  }
  return _shafft::planCart(
      plan, ndim, const_cast<int *>(COMM_DIMS.data()),
      const_cast<int *>(dimensions.data()), precision, COMM);
}

int destroy(PlanData **plan) { return _shafft::destroy(plan); }
...
int normalize(PlanData *plan) { return _shafft::normalize(plan); }
...
int allocBuffer(size_t count, complexf** buf) noexcept {
  if (!buf) return static_cast<int>(Status::SHAFFT_ERR_NULLPTR);
  if (count == 0) {
    *buf = nullptr;
  ...
  *buf = static_cast<complexf*>(std::malloc(count * sizeof(complexf)));
  if (!*buf) return static_cast<int>(Status::SHAFFT_ERR_ALLOC);
#endif
  return static_cast<int>(Status::SHAFFT_SUCCESS);
}
...
int freeBuffer(complexf* buf) noexcept {
  if (!buf) return static_cast<int>(Status::SHAFFT_SUCCESS);
#if SHAFFT_BACKEND_HIPFFT
  hipError_t err = hipFree(buf);
  if (err != hipSuccess) return static_cast<int>(Status::SHAFFT_ERR_BACKEND);
#else
  std::free(buf);
#endif
  return static_cast<int>(Status::SHAFFT_SUCCESS);
...
int copyToBuffer(complexf* dst, const complexf* src, size_t count) noexcept {
  if (!dst || !src) return static_cast<int>(Status::SHAFFT_ERR_NULLPTR);
  if (count == 0) return static_cast<int>(Status::SHAFFT_SUCCESS);
#if SHAFFT_BACKEND_HIPFFT
