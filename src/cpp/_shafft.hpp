...
int setBuffers(shafft::PlanData* plan, T* data, T* work) noexcept {
  try {
    if (!plan)           return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
    if (!plan->slab)     return (int)shafft::Status::SHAFFT_ERR_PLAN_NOT_INIT;
    if (!data || !work)  return (int)shafft::Status::SHAFFT_ERR_NULLPTR;
...
int getBuffers(shafft::PlanData* plan, T** data, T** work) noexcept {
  try {
    ...
    if (!d || !w)        return (int)shafft::Status::SHAFFT_ERR_NO_BUFFER;
...
int configurationNDA(int ndim, int *size, int *nda, int *subsize, int *offset,
                     int *COMM_DIMS, shafft::FFTType precision,
                     size_t mem_limit, MPI_Comm COMM) noexcept;

int configurationCart(int ndim, int *size, int *subsize, int *offset,
                      int *COMM_DIMS, int *COMM_SIZE, shafft::FFTType precision,
                      size_t mem_limit, MPI_Comm COMM) noexcept;
...
int getLayout(shafft::PlanData* plan, int *subsize, int *offset,  shafft::TensorLayout layout) noexcept;
