...
static void get_transform_axes(int *coord_spaces, int nca, int *ca, int *nta,
                               int *ta) {
  *nta = 0;
  for (int i = 0; i < nca; i++) {
...
int setStream(shafft::PlanData *plan, hipStream_t stream) noexcept {
  try {
    if (!plan)            SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    if (!plan->subplans)  SHAFFT_FAIL(SHAFFT_ERR_PLAN_NOT_INIT);
...
int planCreate(shafft::PlanData **plan) noexcept {
  ...
  *plan = new (std::nothrow) shafft::PlanData;
  if (!*plan) { SHAFFT_FAIL_WITH(SHAFFT_ERR_ALLOC, SHAFFT_ERRSRC_SYSTEM, 0); }
  return SHAFFT_STATUS(SHAFFT_SUCCESS);
}
...
int planNDA(shafft::PlanData *plan, int ndim, int nda, int dimensions[],
            shafft::FFTType precision, MPI_Comm COMM) noexcept {
  try {
    if (!plan)          SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    if (!dimensions)    SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    if (ndim <= 0)      SHAFFT_FAIL(SHAFFT_ERR_INVALID_DIM);
    for (int i = 0; i < ndim; ++i)
      if (dimensions[i] <= 0) SHAFFT_FAIL(SHAFFT_ERR_INVALID_DIM);
    // nda must leave at least 1 contiguous axis (nca = ndim - nda >= 1)
    if (nda < 0 || nda >= ndim)     SHAFFT_FAIL(SHAFFT_ERR_INVALID_DECOMP);
    if (COMM == MPI_COMM_NULL)      SHAFFT_FAIL(SHAFFT_ERR_INVALID_COMM);
    ...
    if (nda > 0) {
      ...
      // Cap each axis by its tensor size
      for (int i = 0; i < nda; ++i) {
        COMM_DIMS[i] = std::min(mpi_dims[i], dimensions[i]);
      }
...
int planCart(shafft::PlanData *plan, int ndim, int COMM_DIMS[], int dimensions[],
  ...
  try {
    if (!plan)                    SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    if (!COMM_DIMS || !dimensions)SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    if (ndim <= 0)                SHAFFT_FAIL(SHAFFT_ERR_INVALID_DIM);
    if (COMM_DIMS[ndim - 1] != 1) SHAFFT_FAIL(SHAFFT_ERR_INVALID_DECOMP);
    for (int i = 0; i < ndim; ++i)
    ...
    switch (precision) {
      case FFTType::C2C:
        ...
        plan->slab = new (std::nothrow) Slab(ndim, dimensions, COMM_DIMS, MPI_C_COMPLEX, COMM, elem_size);
        break;
      case FFTType::Z2Z:
        plan->fft_type = FFTType::Z2Z;
        plan->slab = new (std::nothrow) Slab(ndim, dimensions, COMM_DIMS, MPI_C_DOUBLE_COMPLEX, COMM, elem_size);
        break;
      default: SHAFFT_FAIL(SHAFFT_ERR_INVALID_FFTTYPE);
    }
    if (!plan->slab) { SHAFFT_FAIL_WITH(SHAFFT_ERR_ALLOC, SHAFFT_ERRSRC_SYSTEM, 0); }

    // Handle inactive ranks: no FFT subplans needed, just warn and return success
    if (!plan->slab->is_active()) {
      int world_rank = 0;
      MPI_Comm_rank(COMM, &world_rank);
      std::fprintf(stderr, "[SHAFFT] Warning: rank %d is inactive and will not participate in FFT computation.\n", world_rank);
      plan->nsubplans = 0;
    ...
    plan->subplans  = new (std::nothrow) fftHandle[plan->nsubplans];
    if (!plan->subplans) { SHAFFT_FAIL_WITH(SHAFFT_ERR_ALLOC, SHAFFT_ERRSRC_SYSTEM, 0); }

    int subsize[ndim], offset[ndim];
    const int nca = plan->slab->nca();
    int ca[nca], ta[ndim], nta;
    int coord_spaces[ndim]; for (int i = 0; i < ndim; ++i) coord_spaces[i] = 0;

    for (int i = 0; i < plan->nsubplans; ++i) {
      int cfg_rc = plan->slab->get_ith_config(subsize, offset, ca, i);
      if (cfg_rc != 0) return cfg_rc;
...
int destroy(shafft::PlanData **plan) noexcept {
  try {
    if (!plan || !*plan) SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    ...
    if (p->subplans) {
      for (int i = 0; i < p->nsubplans; ++i) {
        ...
        if (rc != 0) {
#if SHAFFT_BACKEND_HIPFFT
          shafft::detail::set_last_error(shafft::Status::SHAFFT_ERR_BACKEND,
                                         SHAFFT_ERRSRC_HIPFFT, rc);
#else
          shafft::detail::set_last_error(shafft::Status::SHAFFT_ERR_BACKEND,
                                         SHAFFT_ERRSRC_FFTW, rc);
#endif
        }
      }
      delete[] p->subplans;
...
int getLayout(PlanData* plan, int* subsize, int* offset, shafft::TensorLayout layout) noexcept {
  try {
    if (!plan || !plan->slab)         SHAFFT_FAIL(SHAFFT_ERR_PLAN_NOT_INIT);
    if (!subsize || !offset)          SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);

    Slab* s = plan->slab;
    int rc = 0;
    switch (layout) {
      case TensorLayout::CURRENT:     s->get_subsize(subsize); s->get_offset(offset); break;
      case TensorLayout::INITIAL:     rc = s->get_ith_layout(subsize, offset, 0);          break;
      case TensorLayout::TRANSFORMED: rc = s->get_ith_layout(subsize, offset, s->nes());   break;
    }
    if (rc != 0) return rc;
    return SHAFFT_STATUS(SHAFFT_SUCCESS);
  }
...
int getAxes(PlanData* plan, int* ca, int* da, shafft::TensorLayout layout) noexcept {
  try {
    if (!plan || !plan->slab)         SHAFFT_FAIL(SHAFFT_ERR_PLAN_NOT_INIT);
    if (!ca || !da)                   SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);

    Slab* s = plan->slab;
    int rc = 0;
    switch (layout) {
      case TensorLayout::CURRENT:     s->get_ca(ca); s->get_da(da);                   break;
      case TensorLayout::INITIAL:     rc = s->get_ith_axes(ca, da, 0);                     break;
      case TensorLayout::TRANSFORMED: rc = s->get_ith_axes(ca, da, s->nes());              break;
    }
    if (rc != 0) return rc;
    return SHAFFT_STATUS(SHAFFT_SUCCESS);
  }
...
int getAllocSize(shafft::PlanData *plan, size_t *alloc_size) noexcept {
  try {
    if (!alloc_size)         SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    if (!plan || !plan->slab) SHAFFT_FAIL(SHAFFT_ERR_PLAN_NOT_INIT);
...
int execute(shafft::PlanData *plan, shafft::FFTDirection direction) noexcept {
  try {
    if (!plan)              SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    if (!plan->slab)        SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    ...
    if (nsp < 1)            SHAFFT_FAIL(SHAFFT_ERR_PLAN_NOT_INIT);
    ...
    for (int i = 0; i < nsp; ++i) {
      ...
      if (!data || !work)   SHAFFT_FAIL(SHAFFT_ERR_NO_BUFFER);

      SHAFFT_BACKEND_CHECK(fftExecute(plan->subplans[subindex], data, work, direction));

      const bool result_in_work = (plan->subplans[subindex].nsubplans % 2) == 1;
      if (result_in_work) plan->slab->swap_buffers();
    ...
    plan->current_layout = (plan->current_layout == TensorLayout::INITIAL)
                           ? TensorLayout::CURRENT
...
int normalize(shafft::PlanData *plan) noexcept {
  try {
    if (!plan)             SHAFFT_FAIL(SHAFFT_ERR_NULLPTR);
    if (!plan->slab)       SHAFFT_FAIL(SHAFFT_ERR_PLAN_NOT_INIT);
    ...
    switch (plan->fft_type) {
      case FFTType::C2C: norm_rc = normalizeComplexFloat ((float)  norm_factor, tensor_size, data); break;
      case FFTType::Z2Z: norm_rc = normalizeComplexDouble((double) norm_factor, tensor_size, data); break;
    }
    if (norm_rc != 0) {
#if SHAFFT_BACKEND_HIPFFT
