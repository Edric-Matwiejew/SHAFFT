...
static bool test_buffers_swap_after_forward() {
    ...
    (void)plan.getBuffers(&before_data, &before_work);
    
    // Execute forward
    (void)plan.execute(shafft::FFTDirection::FORWARD);
    
    shafft::complexf *after_data, *after_work;
    (void)plan.getBuffers(&after_data, &after_work);
...
static bool test_data_buffer_has_result() {
    ...
    (void)shafft::copyToBuffer(work, zeros.data(), n);
...
static bool test_roundtrip_buffer_tracking() {
    ...
    // Read result
    std::vector<shafft::complexf> result(n);
    (void)shafft::copyFromBuffer(result.data(), final_data, n);
