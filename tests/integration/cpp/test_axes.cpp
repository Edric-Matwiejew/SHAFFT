...
static bool test_axes_complete_coverage() {
  ...
  // Create commDims for nda=1: distribute first axis
...
static bool test_initial_axes() {
  ...
  if (worldSize > 1) {
    ...
    ca_set.insert(ca[0]);
    ca_set.insert(ca[1]);
    if (ca_set.find(1) == ca_set.end())
      return false;
    if (ca_set.find(2) == ca_set.end())
      return false;
  }
...
// Test: REDISTRIBUTED layout - distributed axis shifts
static bool test_redistributed_axes() {
  std::vector<size_t> dims = {64, 64, 32};
  const int ndim = static_cast<int>(dims.size());
  // nda = 1
  ...
  rc = fft.getAxes(init_ca, init_da, shafft::TensorLayout::INITIAL);
  if (rc != 0)
    return false;
  rc = fft.getAxes(trans_ca, trans_da, shafft::TensorLayout::REDISTRIBUTED);
  if (rc != 0)
    return false;

  // For nda=1 with multi-rank:
  // INITIAL: da[0] = 0
  // REDISTRIBUTED: distribution should shift (typically to last axis)

  // At minimum, verify redistributed has valid axes
  // (da[0] should be a valid axis index)
  if (trans_da[0] < 0 || trans_da[0] >= ndim)
    return false;
...
static bool test_current_axes_tracks_state() {
  ...
  if (rc != 0)
    return false;
  rc = fft.getAxes(curr_ca, curr_da, shafft::TensorLayout::CURRENT);
  if (rc != 0)
    return false;

  // Compare first nda distributed axes
  if (init_da[0] != curr_da[0])
    return false;
  ...
  // After forward: CURRENT should match REDISTRIBUTED
  std::vector<int> trans_ca(ndim), trans_da(ndim);
  rc = fft.getAxes(trans_ca, trans_da, shafft::TensorLayout::REDISTRIBUTED);
  if (rc != 0) {
    (void)shafft::freeBuffer(data);
...
static bool test_axes_match_layout() {
  ...
  rc = fft.getLayout(subsize, offset, shafft::TensorLayout::INITIAL);
  if (rc != 0)
    return false;
  rc = fft.getAxes(ca, da, shafft::TensorLayout::INITIAL);
  if (rc != 0)
