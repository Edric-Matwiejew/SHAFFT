...
static bool test_z2z_roundtrip() {
  ...
  for (size_t i = 0; i < localElems; ++i) {
    ...
    original[i] = is_origin ? shafft::complexd{1.0, 0.0} : shafft::complexd{0.0, 0.0};
  }
  ...
  bool passed = true;
  for (size_t i = 0; i < localElems; ++i) {
    if (!test::approx_eq(result[i], original[i], 1e-10)) {
      passed = false;
      break;
    }
  }
...
static bool test_z2z_delta() {
  ...
  for (size_t i = 0; i < trans_local; ++i) {
    if (!test::approx_eq(result[i], expected, 1e-10)) {
      passed = false;
      break;
    }
  }
...
int main(int argc, char** argv) {
  ...
  test::TestRunnerMPI runner("Double Precision (Z2Z) Tests");
