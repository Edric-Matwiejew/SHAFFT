...
int main(int argc, char** argv) {
  (void)argc;
  (void)argv;
  std::printf("test_transpose_fft: SKIPPED (hipFFT backend not built)\n");
  return 0;
}
#else
...
int main(int argc, char** argv) {
  ...
  (void)ndim;                     // Used implicitly in array sizes
  ...
  // ===== Reference: direct strided FFT with superbatch iteration =====
  {
    ...
    size_t worksize;
    HIPFFT_CHECK(hipfftMakePlanMany64(
        plan, 2, n, n, istride, idist, n, istride, idist, HIPFFT_Z2Z, batch, &worksize));
    ...
    for (int k = 0; k < 2; ++k) {
      hipfftDoubleComplex* ptr =
          reinterpret_cast<hipfftDoubleComplex*>(d_work) + k * superbatch_offset;
    ...
    HIP_CHECK(
        hipMemcpy(h_reference.data(), d_work, total_size * sizeof(Complex), hipMemcpyDeviceToHost));
  ...
  {
    HIP_CHECK(
        hipMemcpy(d_data, h_input.data(), total_size * sizeof(Complex), hipMemcpyHostToDevice));
    ...
    GPUTT_CHECK(gputtPlan(&transpose_fwd,
                          complex_rank,
                          complex_dims,
                          perm_fwd,
                          gputtDataTypeFloat64,
                          (gputtStream)stream));
    GPUTT_CHECK(gputtPlan(&transpose_bwd,
                          complex_rank,
                          transposed_dims,
                          perm_bwd,
                          gputtDataTypeFloat64,
    ...
    // Check round-trip
    HIP_CHECK(
        hipMemcpy(h_output.data(), d_work, total_size * sizeof(Complex), hipMemcpyDeviceToHost));
    double max_err = 0.0;
    for (long long i = 0; i < total_size; ++i) {
  ...
  // Compare FFT on transposed data vs reference approach
  {
    ...
    size_t worksize;
    HIPFFT_CHECK(hipfftMakePlanMany64(
        fft_plan, 2, n_t, n_t, 1LL, 12LL, n_t, 1LL, 12LL, HIPFFT_Z2Z, batch_t, &worksize));
    ...
    HIP_CHECK(
        hipMemcpy(d_work, h_reference.data(), total_size * sizeof(Complex), hipMemcpyHostToDevice));
    ...
    HIP_CHECK(
        hipMemcpy(h_test.data(), d_temp, total_size * sizeof(Complex), hipMemcpyDeviceToHost));
    HIP_CHECK(hipMemcpy(
        h_ref_transposed.data(), d_data, total_size * sizeof(Complex), hipMemcpyDeviceToHost));
    ...
    printf(
        "FFT transposed vs reference transposed max error: %.6e at index %lld\n", max_err, max_idx);
    if (max_err > 1e-10) {
      printf("  Reference: (%.6f, %.6f)\n",
             h_ref_transposed[max_idx].real(),
             h_ref_transposed[max_idx].imag());
  ...
  // ===== Test: transpose-FFT-transpose approach =====
  {
    ...
    size_t worksize;
    HIPFFT_CHECK(hipfftMakePlanMany64(fft_plan,
                                      2,
                                      n_t,
                                      n_t,
                                      1LL,
                                      12LL, // trailing axes: istride=1, idist=fft_size
                                      n_t,
                                      1LL,
                                      12LL,
                                      HIPFFT_Z2Z,
  ...
  HIP_CHECK(
      hipMemcpy(h_output.data(), d_data, total_size * sizeof(Complex), hipMemcpyDeviceToHost));
